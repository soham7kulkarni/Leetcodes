...
# First check if array is rotated or not by comparing values of low and high. If array is not rotated it means low has pivot. 
# If it is rotated, calculate mid. Handle edge cases for both sides. 
# If middle is on left, cant comapre with another left element. If its on right side we already got it covered by stating if low <= high at start of while loop
